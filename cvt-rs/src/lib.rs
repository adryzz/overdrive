...
impl CvtTimings {
    ...
    pub fn generate(h_pixels: u32, v_pixels: u32, refresh_rate: f64, blanking_mode: BlankingMode, margins: bool, interlaced: bool) -> Result<Self, anyhow::Error> {
        ...
        let v_field_rate_rqd = if interlaced { refresh_rate * 2.0 } else { refresh_rate };

        let h_pixels_rnd = (h_pixels as f64 / cell_gran_rnd).floor() * cell_gran_rnd;

        // if using margins, set the margin
        let left_margin = if margins {((h_pixels_rnd * margin_per / 100.0) / cell_gran_rnd).floor() * cell_gran_rnd} else {0.0};
        let right_margin = left_margin;

        let total_active_pixels = (h_pixels_rnd + left_margin + right_margin).floor() as u32; // floor?

        let v_lines_rnd = if interlaced { ((v_pixels as f64) / 2.0).floor() } else { (v_pixels as f64).floor() };
        
        let top_margin = if margins {(v_lines_rnd * margin_per / 100.0).floor()} else {0.0};
        ...
        let aspect_ratio = get_aspect_ratio(interlaced, v_lines_rnd, h_pixels_rnd, cell_gran_rnd);
        ...
        if blanking_mode == BlankingMode::Normal {
           h_period_est = ((1.0 / v_field_rate_rqd) - min_vsync_bp / 1000000.0) / (v_lines_rnd + (2.0 * top_margin) + min_v_porch_rnd + interlace) * 1000000.0;
           ...
           if ideal_duty_cycle < 20.0 {
            h_blank = (total_active_pixels as f64 * 20.0 / (100.0-20.0) / (2.0 * cell_gran_rnd)) * (2.0 * cell_gran_rnd);
           } else {
            h_blank = (total_active_pixels as f64 * ideal_duty_cycle / (100.0 - ideal_duty_cycle) / (2.0 * cell_gran_rnd)).floor() * (2.0 * cell_gran_rnd);
           }
           ...
           act_pix_freq = clock_step * (total_pixels / h_period_est / clock_step).floor();
            ...
            h_period_est = ((1000000.0 / v_field_rate_rqd) - rb_min_v_blank as f64) / (v_lines_rnd + top_margin + bot_margin);
            ...
            let act_vbi_lines = if vbi_lines < rb_min_vbi {rb_min_vbi} else {vbi_lines};
...
fn get_aspect_ratio(interlaced: bool, v_lines_rnd: f64, h_pixels_rnd: f64, cell_gran_rnd: f64) -> AspectRatio {
    let ver_pixels = if interlaced {2.0 * v_lines_rnd} else {v_lines_rnd};
